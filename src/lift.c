...
{
	//Creat the needed Queue
	QueueHandle_t xJobsA, xJobsB, xMessages;
	xJobsA = xQueueCreate( 10, sizeof( unsigned long ) );
	if( xJobsA == 0 )
	{
	        // Failed to create the queue.
		LCD_DisplayStringXY(10, 10, "Failed to create xJobA");
	}

	xJobsB = xQueueCreate( 10, sizeof( unsigned long ) );
	if( xJobsB == 0 )
	{
	        // Failed to create the queue.
		LCD_DisplayStringXY(10, 10, "Failed to create xJobB");
	}

	xMessages = xQueueCreate(10, sizeof( CARME_CAN_MESSAGE));
	if( xMessages == 0 )
	{
	        // Failed to create the queue.
		LCD_DisplayStringXY(10, 10, "Failed to create xMessage");
	}
	...
	for(;;)
	{
		...
		if( xQueueReceive( xJobsA,  rxA , ( TickType_t ) 10 )) //writes the Message of the queue into rxLevelA
		 {
	...
	/*		if(directionA == STILL)//Not moving
		...
		//1b)-------------------------------------------------------------------------------------------------------------------------
		 if( xQueueReceive( xJobsB,  rxB , ( TickType_t ) 10 ) ) //writes the Message of the queue into rxLevelA
		 {
